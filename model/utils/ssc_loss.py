...
def bev_sem_loss(pred, ssc_target):
    pred = F.softmax(pred, dim=1)
    loss = 0
    count = 0
    mask = (ssc_target != 255) and (ssc_target != 0)
...
def sem_scal_loss(pred, ssc_target):
    ...
    for i in range(0, n_classes):
        ...
        if torch.sum(completion_target) > 0:
            ...
            if i == 0:
                empty_loss += loss_class
            else:
                class_loss += loss_class
            loss += loss_class
    return loss / count, empty_loss, class_loss / count
...
def CE_prob_ssc_loss(pred, target_prob, target, class_weights):
    criterion = nn.CrossEntropyLoss(
        weight=class_weights, reduction="none"
    )
    target_prob = target_prob.permute(0, 3, 1, 2)
    loss = criterion(pred, target_prob)
    loss_valid = loss[target!=255]
    loss_valid_mean = torch.mean(loss_valid)
    return loss_valid_mean
