...
class SpatialCrossAttention(nn.Module):
    def __init__(self, args, status = "normal", dtype = torch.float, dims=None, level=None):
        ...
        self.deformable_attention = DeformableAttention(args, status = status, dims = dims, level=self.level)
    ...
    def forward(self, query, key, value,
        ...
        if fb_mask is not None:
            reference_points_sampling = torch.zeros_like(reference_points_rebatch)
            bs, cam, hight, width = fb_mask.shape
            reference_points_sampling[..., 0] = reference_points_rebatch[..., 0] * width
            reference_points_sampling[..., 1] = reference_points_rebatch[..., 1] * hight
            reference_points_sampling = torch.mean(reference_points_sampling, dim=3).to(int)
            query_map_single = fb_mask[:, :, reference_points_sampling[0, 0, :, 1], reference_points_sampling[0, 0, :, 0]]
            query_map_single[query_map_single==2] = 0
            query_map_single[query_map_single==1] = 1
            query_map_single = query_map_single.to(torch.float32).to(query.device)
        ...
        value = value.permute(2, 0, 1, 3).reshape(
            bs * self.num_cam, l, self.dims)
        queries, sampling_locations = self.deformable_attention(queries_rebatch.view(bs * self.num_cam, max_len, self.dims),
                                     key,
                                     value,
                                     reference_points=reference_points_rebatch.view(bs * self.num_cam, max_len, D, 2),
                                     spatial_shapes=spatial_shapes,
                                     level_start_index=level_start_index)
        queries = queries.view(bs, self.num_cam, max_len, self.dims)
        # sampling_locations = torch.mean(sampling_locations.permute(0, 1, 5, 2, 3, 4).flatten(3), dim=3).view(bs, self.num_cam, max_len, 2)
        # sampling_locations_full = sampling_locations.new_zeros(bs, self.num_cam, num_query, 2)
        query_map_full = sampling_locations.new_ones(bs, self.num_cam, num_query)
        for j in range(bs):
            for i, index_query_per_img in enumerate(indexes):
                slots[j, index_query_per_img] += queries[j, i, :len(index_query_per_img)]
                if fb_mask is not None:
                    query_map_full[j, i, index_query_per_img] = query_map_single[j, i, :len(index_query_per_img)]
                # sampling_locations_full[j, i, index_query_per_img] = sampling_locations[j, i, :len(index_query_per_img)]
        ...
        if fb_mask is not None:
            return self.dropout(slots) + inp_residual, query_map_full
...
class DeformableAttention(nn.Module):
    def __init__(self, args, status = "normal", dtype = torch.float, dims=None, level=None):
        ...
        elif status == "fov" or "sdv":
            self.all_sampling_points = 64
    ...
    def init_weights(self):
        ...
        grid_init = (grid_init /
            ...
            self.head_num, 1, 1,
            2).repeat(1, self.num_levels, self.all_sampling_points, 1)
    ...
    def forward(self, query, key, value,
        ...
        # print("sampling_locations: ", sampling_offsets)
