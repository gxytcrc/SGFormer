...
def tensor_to_vis(tensor, window_name):
    image_vis = tensor.squeeze(0).permute(1, 2, 0).cpu().numpy()
    image_vis = image_vis[:, :, ::-1]
    cv.imshow(window_name, image_vis)
    cv.waitKey(1)

def get_filename_by_id(folder_path):
...
def bev_compress(semantic_tensor, num_class):
    bs, h, w, z = semantic_tensor.shape
    semantic_tensor = semantic_tensor.clone()
    semantic_tensor = semantic_tensor.squeeze(0)
    num_labels = num_class  # Assuming labels are [0, num_labels-1]
    valid_labels_mask = (semantic_tensor != 0) & (semantic_tensor != 255)
    semantic_tensor = torch.where(valid_labels_mask, semantic_tensor, torch.tensor(0, device=semantic_tensor.device))
    # Step 2: Adjust tensor for one-hot encoding, excluding the excluded labels by considering the valid range
    # Note: If setting excluded labels to -1, ensure they don't contribute to one-hot encoding by starting encoding from 1
    adjusted_tensor = semantic_tensor.to(torch.int64)
    # Step 3: Perform one-hot encoding and count occurrences, now with adjusted label indices
    one_hot = torch.zeros(h, w, z, num_labels, device=semantic_tensor.device)  # num_labels - 1 to exclude label 255
    one_hot.scatter_(-1, adjusted_tensor.unsqueeze(-1), 1)
    # Sum across the z dimension to count occurrences of each label, excluding the first "label" which now represents excluded values
    label_counts = one_hot.sum(dim=2)[:, :, 1:]  # Exclude the first column which represents the -1 set earlier
    # Find the indices of the most frequent labels, adjusting back to original label values
    count, bev_tensor = label_counts.max(dim=-1)
    mask = count != 0
    max_count = torch.where(mask, count, torch.tensor(1000, device=bev_tensor.device))
    prob = label_counts / max_count.unsqueeze(-1)
    prob_0 = torch.zeros(prob.shape[0], prob.shape[1], 1).to(prob.device)
    bev_prob = torch.dstack((prob_0, prob))
    bev_tensor = torch.where(mask, bev_tensor, torch.tensor(254, device=bev_tensor.device))
    bev_tensor += 1
    return bev_tensor, bev_prob
